...
import Y00tsBoothSuit from "@/components/ArtBooth/Y00tsBoothSuit";
...
import Y00tsBoothMemecoin from "@/components/ArtBooth/Y00tsBoothMemecoin";
import Y00tsBoothJenner from "@/components/ArtBooth/Y00tsBoothJenner";
import { SetStateAction, useState } from "react";
import ArtistDropdown from "@/components/ArtistDropdown";
import NavBar from '@/components/Site/Navnew';

export default function Home() {
    const [toggleValue, setToggleValue] = useState("Option 1");
    const [toggleValue2, setToggleValue2] = useState("Option A");
    ...
    return (
        <main className={`flex min-h-screen w-full flex-col items-center justify-start pt-8`}>
            ...
            <div className="container flex w-40 sm:right-[380px]" style={{ position: 'absolute', zIndex: 2 }}>
                ...
                <div className="w-40">
                    {/* Primary Toggle */}
                    <div className={`w-full flex justify-center mb-2 sm:relative absolute sm:top-[-160px]`}>
                        <div className="w-96 h-15 bg-greywhite bg-opacity-50 sm:px-2 sm:py-2 px-2 py-2 rounded-xl border-2 border-greywhite border-opacity-50">
                            <div className="flex space-x-3 relative justify-center">
                                <button
                                    className={`w-28 h-10 sm:px-6 sm:py-2 px-7 py-2.5 text-white rounded-md ${toggleValue === "Option 1" ? "selected bg-slate text-white" : ""}`}
                                    onClick={() => {
                        ...
                        <div className={`w-full flex mx-auto justify-center mb-2 top-[70px] sm:top-[-90px]`} style={{ position: 'absolute' }}>
                            <div className="w-96 h-15 bg-greywhite bg-opacity-50 sm:px-2 sm:py-2 px-2 py-2 rounded-xl border-2 border-greywhite border-opacity-50">
                                <div className="flex space-x-3 relative justify-center">
                                    ...
                                    <button
                                        className={`w-28 h-10 sm:px-6 sm:py-2 px-7 py-2.5 text-white rounded-md ${toggleValue2 === "Option C" ? "selected bg-slate text-green" : ""}`}
                                        onClick={() => handleToggleChange2("Option C")}
                                        style={{}}>
                                        <a className={`text-sm flex mx-auto justify-center`}>Memecoins</a>
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className={`w-full flex mx-auto justify-center mb-2 top-[70px] sm:top-[-90px]`} style={{ position: 'absolute' }}>
                            <div className="w-96 h-15 bg-greywhite bg-opacity-50 sm:px-2 sm:py-2 px-2 py-2 rounded-xl border-2 border-greywhite border-opacity-50">
                                <div className="flex space-x-3 relative justify-center">
                                    ...
                                    <button
                                        className={`w-28 h-10 sm:px-6 sm:py-2 px-7 py-2.5 text-white rounded-md ${toggleValue2 === "Check 1" ? "selected bg-slate from-forbes-light to-forbes-dark text-green" : ""}`}
                                        onClick={() => handleToggleChange2("Check 1")}
                                        style={{}}>
                                        <a className={`text-sm flex mx-auto justify-center`}>Jenner</a>
                                    </button>
                                    <button
            ...
            <div className={`container sm:pt-10 pt-24 mt-3 sm:mt-0`}>
                ...
                {toggleValue2 === "Option B" && <Y00tsBoothSuit />}
                {toggleValue2 === "Option C" && <Y00tsBoothMemecoin />}


                {/* Content based on third toggle */}
                {toggleValue2 === "Check 1" && <Y00tsBoothJenner />}
